...
public class ComplaintCategoriesTableInternalView extends JInternalFrame {
	 ...
	 * Create the frame.
	 */
	public ComplaintCategoriesTableInternalView(RepresentativeDashboardController representativeDashboardController) {
		super("Complaint Categories", true, true, true, true);
		...
		TimerTask task = new TimerTask() {
	        public void run() {
	        	representativeDashboardController.sendQueryToModel("READ_ALL_COMPLAINT_CATEGORIES");
	        }
	    };
	...
	private void initalizeComponents() {
		...
		btnRefresh.setIcon(new ImageIcon(ComplaintCategoriesTableInternalView.class.getResource("/Images/icons8-refresh-document-25.png")));
		btnRefresh.setFocusable(false);
		btnRefresh.setBounds(10, 460, 140, 30);
		
		btnInsert = new JButton("Insert");
		btnInsert.setIcon(new ImageIcon(ComplaintCategoriesTableInternalView.class.getResource("/Images/icons8-insert-button-25.png")));
		btnInsert.setFocusable(false);
		btnInsert.setBounds(160, 460, 140, 30);
		
		btnUpdate = new JButton("Update");
		btnUpdate.setIcon(new ImageIcon(ComplaintCategoriesTableInternalView.class.getResource("/Images/icons8-update-25.png")));
		btnUpdate.setFocusable(false);
		btnUpdate.setBounds(310, 460, 140, 30);
		
		btnDelete = new JButton("Delete");
		btnDelete.setIcon(new ImageIcon(ComplaintCategoriesTableInternalView.class.getResource("/Images/icons8-delete-bin-30.png")));
		btnDelete.setFocusable(false);
		btnDelete.setBounds(460, 460, 140, 30);
		
		btnNewRow = new JButton("New Row");
		btnNewRow.setIcon(new ImageIcon(ComplaintCategoriesTableInternalView.class.getResource("/Images/icons8-plus-25.png")));
	...
	public void populateTable(ArrayList<ComplaintCategoriesHibernate> complaintCategoriesList) {
		DefaultTableModel tableModel = (DefaultTableModel) table.getModel();
		tableModel.setRowCount(0); // clear table

		for (ComplaintCategoriesHibernate value : complaintCategoriesList) {
			tableModel.addRow(new Object[] { value.getId(), value.getCategoryId(), value.getCategoryName() });
		} 
	}
	...
	private void insert() {
		...
		if (table.getValueAt(index, 0) == null) {
			if (validateDatatypeInCells() && validateForEmptyCells()) {
				ComplaintCategoriesHibernate complaintCategoriesHibernateObj = new ComplaintCategoriesHibernate();
				complaintCategoriesHibernateObj.setCategoryId(Integer.parseInt(table.getValueAt(index, 1).toString()));
				complaintCategoriesHibernateObj.setCategoryName(table.getValueAt(index, 2).toString());
				
				representativeDashboardController.sendObjectToModel("INSERT_COMPLAINT_CATEGORY");
				representativeDashboardController.sendObjectToModel(complaintCategoriesHibernateObj);
	...
	private void update() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				if (validateDatatypeInCells() && validateForEmptyCells()) {
					ComplaintCategoriesHibernate complaintCategoriesHibernateObj = new ComplaintCategoriesHibernate();
					complaintCategoriesHibernateObj.setId(Integer.parseInt(table.getValueAt(index, 0).toString()));
					complaintCategoriesHibernateObj.setCategoryId(Integer.parseInt(table.getValueAt(index, 1).toString()));
					complaintCategoriesHibernateObj.setCategoryName(table.getValueAt(index, 2).toString());
					
					representativeDashboardController.sendObjectToModel("UPDATE_COMPLAINT_CATEGORY");
					representativeDashboardController.sendObjectToModel(complaintCategoriesHibernateObj);
	...
	private void delete() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				...
				representativeDashboardController.sendObjectToModel("DELETE_COMPLAINT_CATEGORY");
