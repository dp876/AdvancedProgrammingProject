...
public class CustomerLoginTableInternalView extends JInternalFrame {
	 ...
	 * Create the frame.
	 */
	public CustomerLoginTableInternalView(RepresentativeDashboardController representativeDashboardController) {
		super("Customer Login", true, true, true, true);
		...
		TimerTask task = new TimerTask() {
	        public void run() {
	        	representativeDashboardController.sendQueryToModel("READ_ALL_CUSTOMER_LOGIN");
	        }
	    };
	...
	private void initalizeComponents() {
		...
		table.setModel(new DefaultTableModel(
				...
				new String[] {
					"Id", "Customer Id", "Customer Password"
				}
			) {
				/**
				 * 
		...
		btnRefresh.setIcon(new ImageIcon(CustomerLoginTableInternalView.class.getResource("/Images/icons8-refresh-document-25.png")));
		btnRefresh.setFocusable(false);
		btnRefresh.setBounds(10, 460, 140, 30);
		
		btnInsert = new JButton("Insert");
		btnInsert.setIcon(new ImageIcon(CustomerLoginTableInternalView.class.getResource("/Images/icons8-insert-button-25.png")));
		btnInsert.setFocusable(false);
		btnInsert.setBounds(160, 460, 140, 30);
		
		btnUpdate = new JButton("Update");
		btnUpdate.setIcon(new ImageIcon(CustomerLoginTableInternalView.class.getResource("/Images/icons8-update-25.png")));
		btnUpdate.setFocusable(false);
		btnUpdate.setBounds(310, 460, 140, 30);
		
		btnDelete = new JButton("Delete");
		btnDelete.setIcon(new ImageIcon(CustomerLoginTableInternalView.class.getResource("/Images/icons8-delete-bin-30.png")));
		btnDelete.setFocusable(false);
		btnDelete.setBounds(460, 460, 140, 30);
		
		btnNewRow = new JButton("New Row");
		btnNewRow.setIcon(new ImageIcon(CustomerLoginTableInternalView.class.getResource("/Images/icons8-plus-25.png")));
	...
	public void populateTable(ArrayList<CustomerLoginHibernate> customerLoginList) {
		DefaultTableModel tableModel = (DefaultTableModel) table.getModel();
		tableModel.setRowCount(0); // clear table

		for (CustomerLoginHibernate value : customerLoginList) {
			tableModel.addRow(new Object[] { value.getId(), value.getCustomerId(), value.getCustomerPassword() });
		} 
	}
	...
	private void insert() {
		...
		if (table.getValueAt(index, 0) == null) {
			if (validateDatatypeInCells() && validateForEmptyCells()) {
				CustomerLoginHibernate customerLoginHibernateObj = new CustomerLoginHibernate();
				customerLoginHibernateObj.setCustomerId(Integer.parseInt(table.getValueAt(index, 1).toString()));
				customerLoginHibernateObj.setCustomerPassword(table.getValueAt(index, 2).toString());
				
				representativeDashboardController.sendObjectToModel("INSERT_CUSTOMER_LOGIN");
				representativeDashboardController.sendObjectToModel(customerLoginHibernateObj);
	...
	private void update() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				if (validateDatatypeInCells() && validateForEmptyCells()) {
					CustomerLoginHibernate customerLoginHibernateObj = new CustomerLoginHibernate();
					customerLoginHibernateObj.setId(Integer.parseInt(table.getValueAt(index, 0).toString()));
					customerLoginHibernateObj.setCustomerId(Integer.parseInt(table.getValueAt(index, 1).toString()));
					customerLoginHibernateObj.setCustomerPassword(table.getValueAt(index, 2).toString());
					
					representativeDashboardController.sendObjectToModel("UPDATE_CUSTOMER_LOGIN");
					representativeDashboardController.sendObjectToModel(customerLoginHibernateObj);
	...
	private void delete() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				...
				representativeDashboardController.sendObjectToModel("DELETE_CUSTOMER_LOGIN");
