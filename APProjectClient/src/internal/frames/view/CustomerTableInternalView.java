...
public class CustomerTableInternalView extends JInternalFrame {
	 ...
	 * Create the frame.
	 */
	public CustomerTableInternalView(RepresentativeDashboardController representativeDashboardController) {
		...
		TimerTask task = new TimerTask() {
	        public void run() {
	        	representativeDashboardController.sendQueryToModel("READ_ALL_CUSTOMER");
	        }
	    };
	...
	private void initalizeComponents() {
		...
		table.setModel(new DefaultTableModel(
				...
				new String[] {
					"Id", "User Id", "Firstname", "Lastname", "Email", "Address", "Tele"
				}
			) {
				/**
				 * 
		...
		btnRefresh.setIcon(new ImageIcon(CustomerTableInternalView.class.getResource("/Images/icons8-refresh-document-25.png")));
		btnRefresh.setFocusable(false);
		btnRefresh.setBounds(10, 460, 140, 30);
		
		btnInsert = new JButton("Insert");
		btnInsert.setIcon(new ImageIcon(CustomerTableInternalView.class.getResource("/Images/icons8-insert-button-25.png")));
		btnInsert.setFocusable(false);
		btnInsert.setBounds(160, 460, 140, 30);
		
		btnUpdate = new JButton("Update");
		btnUpdate.setIcon(new ImageIcon(CustomerTableInternalView.class.getResource("/Images/icons8-update-25.png")));
		btnUpdate.setFocusable(false);
		btnUpdate.setBounds(310, 460, 140, 30);
		
		btnDelete = new JButton("Delete");
		btnDelete.setIcon(new ImageIcon(CustomerTableInternalView.class.getResource("/Images/icons8-delete-bin-30.png")));
		btnDelete.setFocusable(false);
		btnDelete.setBounds(460, 460, 140, 30);
		
		btnNewRow = new JButton("New Row");
		btnNewRow.setIcon(new ImageIcon(CustomerTableInternalView.class.getResource("/Images/icons8-plus-25.png")));
	...
	public void populateTable(ArrayList<CustomerHibernate> customerList) {
		DefaultTableModel tableModel = (DefaultTableModel) table.getModel();
		tableModel.setRowCount(0); // clear table

		for (CustomerHibernate value : customerList) {
			tableModel.addRow(new Object[] { value.getId(), value.getCustomerUserId(), 
					value.getFirstName(), value.getLastName(), value.getEmailAddress(),
					value.getHomeAddress(), value.getTeleNum() });
		} 
	}
	...
	private void insert() {
		...
		if (table.getValueAt(index, 0) == null) {
			if (validateDatatypeInCells() && validateForEmptyCells()) {
				CustomerHibernate customerHibernateObj = new CustomerHibernate();
				customerHibernateObj.setCustomerUserId(Integer.parseInt(table.getValueAt(index, 1).toString()));
				customerHibernateObj.setFirstName(table.getValueAt(index, 2).toString());
				customerHibernateObj.setLastName(table.getValueAt(index, 3).toString());
				customerHibernateObj.setEmailAddress(table.getValueAt(index, 4).toString());
				customerHibernateObj.setHomeAddress(table.getValueAt(index, 5).toString());
				customerHibernateObj.setTeleNum(table.getValueAt(index, 6).toString());
				
				representativeDashboardController.sendObjectToModel("INSERT_CUSTOMER");
				representativeDashboardController.sendObjectToModel(customerHibernateObj);
	...
	private void update() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				if (validateDatatypeInCells() && validateForEmptyCells()) {
					CustomerHibernate customerHibernateObj = new CustomerHibernate();
					customerHibernateObj.setId(Integer.parseInt(table.getValueAt(index, 0).toString()));
					customerHibernateObj.setCustomerUserId(Integer.parseInt(table.getValueAt(index, 1).toString()));
					customerHibernateObj.setFirstName(table.getValueAt(index, 2).toString());
					customerHibernateObj.setLastName(table.getValueAt(index, 3).toString());
					customerHibernateObj.setEmailAddress(table.getValueAt(index, 4).toString());
					...
					representativeDashboardController.sendObjectToModel("UPDATE_CUSTOMER");
					representativeDashboardController.sendObjectToModel(customerHibernateObj);
	...
	private void delete() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				...
				representativeDashboardController.sendObjectToModel("DELETE_CUSTOMER");
