...
public class EmployeeTableInternalView extends JInternalFrame {
	 ...
	 * Create the frame.
	 */
	public EmployeeTableInternalView(RepresentativeDashboardController representativeDashboardController) {
		...
		TimerTask task = new TimerTask() {
	        public void run() {
	        	representativeDashboardController.sendQueryToModel("READ_ALL_EMPLOYEE");
	        }
	    };
	...
	private void initalizeComponents() {
		...
		table.setModel(new DefaultTableModel(
				...
				new String[] {
					"Id", "Employee Type", "User Id", "Firstname", "Lastname", "Email", "Address", "Tele"
				}
			) {
				/**
				 * 
		...
		btnRefresh.setIcon(new ImageIcon(EmployeeTableInternalView.class.getResource("/Images/icons8-refresh-document-25.png")));
		btnRefresh.setFocusable(false);
		btnRefresh.setBounds(10, 460, 140, 30);
		
		btnInsert = new JButton("Insert");
		btnInsert.setIcon(new ImageIcon(EmployeeTableInternalView.class.getResource("/Images/icons8-insert-button-25.png")));
		btnInsert.setFocusable(false);
		btnInsert.setBounds(160, 460, 140, 30);
		
		btnUpdate = new JButton("Update");
		btnUpdate.setIcon(new ImageIcon(EmployeeTableInternalView.class.getResource("/Images/icons8-update-25.png")));
		btnUpdate.setFocusable(false);
		btnUpdate.setBounds(310, 460, 140, 30);
		
		btnDelete = new JButton("Delete");
		btnDelete.setIcon(new ImageIcon(EmployeeTableInternalView.class.getResource("/Images/icons8-delete-bin-30.png")));
		btnDelete.setFocusable(false);
		btnDelete.setBounds(460, 460, 140, 30);
		
		btnNewRow = new JButton("New Row");
		btnNewRow.setIcon(new ImageIcon(EmployeeTableInternalView.class.getResource("/Images/icons8-plus-25.png")));
	...
	public void populateTable(ArrayList<EmployeeHibernate> employeeList) {
		DefaultTableModel tableModel = (DefaultTableModel) table.getModel();
		tableModel.setRowCount(0); // clear table

		for (EmployeeHibernate value : employeeList) {
			tableModel.addRow(new Object[] { value.getId(), value.getEmployeeType(),
					value.getEmployeeUserId(), value.getFirstName(), value.getLastName(),
					value.getEmailAddress(), value.getHomeAddress(), value.getTeleNum() });
		}
	}
	...
	private boolean validateForEmptyCells() {
		...
		if (table.getValueAt(index, 1).toString().isEmpty() || 
				...
				table.getValueAt(index, 6).toString().isEmpty() ||
				table.getValueAt(index, 7).toString().isEmpty()) {
			return false;
		}
		return true;
	}
	...
	private void insert() {
		...
		if (table.getValueAt(index, 0) == null) {
			if (validateDatatypeInCells() && validateForEmptyCells()) {
				...
				employeeHibernateObj.setEmployeeType(table.getValueAt(index, 1).toString());
				employeeHibernateObj.setEmployeeUserId(Integer.parseInt(table.getValueAt(index, 2).toString()));
				employeeHibernateObj.setFirstName(table.getValueAt(index, 3).toString());
				employeeHibernateObj.setLastName(table.getValueAt(index, 4).toString());
				employeeHibernateObj.setEmailAddress(table.getValueAt(index, 5).toString());
				employeeHibernateObj.setHomeAddress(table.getValueAt(index, 6).toString());
				employeeHibernateObj.setTeleNum(table.getValueAt(index, 7).toString());
				
				representativeDashboardController.sendObjectToModel("INSERT_EMPLOYEE");
				representativeDashboardController.sendObjectToModel(employeeHibernateObj);
	...
	private void update() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				if (validateDatatypeInCells() && validateForEmptyCells()) {
					EmployeeHibernate employeeHibernateObj = new EmployeeHibernate();
					employeeHibernateObj.setId(Integer.parseInt(table.getValueAt(index, 0).toString()));
					employeeHibernateObj.setEmployeeType(table.getValueAt(index, 1).toString());
					employeeHibernateObj.setEmployeeUserId(Integer.parseInt(table.getValueAt(index, 2).toString()));
					employeeHibernateObj.setFirstName(table.getValueAt(index, 3).toString());
					employeeHibernateObj.setLastName(table.getValueAt(index, 4).toString());
					...
					representativeDashboardController.sendObjectToModel("UPDATE_EMPLOYEE");
					representativeDashboardController.sendObjectToModel(employeeHibernateObj);
	...
	private void delete() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				...
				representativeDashboardController.sendObjectToModel("DELETE_EMPLOYEE");
