...
public class PaymentHistoryTableInternalView extends JInternalFrame {
	 ...
	 * Create the frame.
	 */
	public PaymentHistoryTableInternalView(RepresentativeDashboardController representativeDashboardController) {
		...
		TimerTask task = new TimerTask() {
	        public void run() {
	        	representativeDashboardController.sendQueryToModel("READ_ALL_PAYMENT_HISTORY");
	        }
	    };
	...
	private void initalizeComponents() {
		...
		table.setModel(new DefaultTableModel(
				...
				new String[] {
					"Id", "Customer Id", "Amount Paid", "Date Due", "Date Paid"
				}
			) {
				/**
				 * 
		...
		btnRefresh.setIcon(new ImageIcon(PaymentHistoryTableInternalView.class.getResource("/Images/icons8-refresh-document-25.png")));
		btnRefresh.setFocusable(false);
		btnRefresh.setBounds(10, 460, 140, 30);
		
		btnInsert = new JButton("Insert");
		btnInsert.setIcon(new ImageIcon(PaymentHistoryTableInternalView.class.getResource("/Images/icons8-insert-button-25.png")));
		btnInsert.setFocusable(false);
		btnInsert.setBounds(160, 460, 140, 30);
		
		btnUpdate = new JButton("Update");
		btnUpdate.setIcon(new ImageIcon(PaymentHistoryTableInternalView.class.getResource("/Images/icons8-update-25.png")));
		btnUpdate.setFocusable(false);
		btnUpdate.setBounds(310, 460, 140, 30);
		
		btnDelete = new JButton("Delete");
		btnDelete.setIcon(new ImageIcon(PaymentHistoryTableInternalView.class.getResource("/Images/icons8-delete-bin-30.png")));
		btnDelete.setFocusable(false);
		btnDelete.setBounds(460, 460, 140, 30);
		
		btnNewRow = new JButton("New Row");
		btnNewRow.setIcon(new ImageIcon(PaymentHistoryTableInternalView.class.getResource("/Images/icons8-plus-25.png")));
	...
	public void populateTable(ArrayList<PaymentHistoryHibernate> paymentHistoryList) {
		DefaultTableModel tableModel = (DefaultTableModel) table.getModel();
		tableModel.setRowCount(0); // clear table

		for (PaymentHistoryHibernate value : paymentHistoryList) {
			tableModel.addRow(new Object[] { value.getId(), value.getCustomerId(),
					value.getAmountPaid(), value.getDueDate(), value.getDatePaid() });
		}
	}
	...
	private void insert() {
		...
		if (table.getValueAt(index, 0) == null) {
			if (validateDatatypeInCells() && validateForEmptyCells()) {
				...
				paymentHistoryHibernateObj.setCustomerId(Integer.parseInt(table.getValueAt(index, 1).toString()));
				paymentHistoryHibernateObj.setAmountPaid(Integer.parseInt(table.getValueAt(index, 2).toString()));
				paymentHistoryHibernateObj.setDueDate(table.getValueAt(index, 3).toString());
				paymentHistoryHibernateObj.setDatePaid(table.getValueAt(index, 4).toString());
				
				representativeDashboardController.sendObjectToModel("INSERT_PAYMENT_HISTORY");
				representativeDashboardController.sendObjectToModel(paymentHistoryHibernateObj);
	...
	private void update() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				if (validateDatatypeInCells() && validateForEmptyCells()) {
					PaymentHistoryHibernate paymentHistoryHibernateObj = new PaymentHistoryHibernate();
					paymentHistoryHibernateObj.setId(Integer.parseInt(table.getValueAt(index, 0).toString()));
					paymentHistoryHibernateObj.setCustomerId(Integer.parseInt(table.getValueAt(index, 1).toString()));
					paymentHistoryHibernateObj.setAmountPaid(Integer.parseInt(table.getValueAt(index, 2).toString()));
					paymentHistoryHibernateObj.setDueDate(table.getValueAt(index, 3).toString());
					paymentHistoryHibernateObj.setDatePaid(table.getValueAt(index, 4).toString());
					
					representativeDashboardController.sendObjectToModel("UPDATE_PAYMENT_HISTORY");
					representativeDashboardController.sendObjectToModel(paymentHistoryHibernateObj);
	...
	private void delete() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				...
				representativeDashboardController.sendObjectToModel("DELETE_PAYMENT_HISTORY");
