...
public class ProductServicesTableInternalView extends JInternalFrame {
	 ...
	 * Create the frame.
	 */
	public ProductServicesTableInternalView(RepresentativeDashboardController representativeDashboardController) {
		...
		TimerTask task = new TimerTask() {
	        public void run() {
	        	representativeDashboardController.sendQueryToModel("READ_ALL_PRODUCT_SERVICES");
	        }
	    };
	...
	private void initalizeComponents() {
		...
		btnRefresh.setIcon(new ImageIcon(ProductServicesTableInternalView.class.getResource("/Images/icons8-refresh-document-25.png")));
		btnRefresh.setFocusable(false);
		btnRefresh.setBounds(10, 460, 140, 30);
		
		btnInsert = new JButton("Insert");
		btnInsert.setIcon(new ImageIcon(ProductServicesTableInternalView.class.getResource("/Images/icons8-insert-button-25.png")));
		btnInsert.setFocusable(false);
		btnInsert.setBounds(160, 460, 140, 30);
		
		btnUpdate = new JButton("Update");
		btnUpdate.setIcon(new ImageIcon(ProductServicesTableInternalView.class.getResource("/Images/icons8-update-25.png")));
		btnUpdate.setFocusable(false);
		btnUpdate.setBounds(310, 460, 140, 30);
		
		btnDelete = new JButton("Delete");
		btnDelete.setIcon(new ImageIcon(ProductServicesTableInternalView.class.getResource("/Images/icons8-delete-bin-30.png")));
		btnDelete.setFocusable(false);
		btnDelete.setBounds(460, 460, 140, 30);
		
		btnNewRow = new JButton("New Row");
		btnNewRow.setIcon(new ImageIcon(ProductServicesTableInternalView.class.getResource("/Images/icons8-plus-25.png")));
	...
	public void populateTable(ArrayList<ProductServicesHibernate> productServicesList) {
		DefaultTableModel tableModel = (DefaultTableModel) table.getModel();
		tableModel.setRowCount(0); // clear table

		for (ProductServicesHibernate value : productServicesList) {
			tableModel.addRow(new Object[] { value.getId(), value.getProductId(), value.getProductName() });
		} 
	}
	...
	private void insert() {
		...
		if (table.getValueAt(index, 0) == null) {
			if (validateDatatypeInCells() && validateForEmptyCells()) {
				...
				productServicesHibernateObj.setProductId(Integer.parseInt(table.getValueAt(index, 1).toString()));
				productServicesHibernateObj.setProductName(table.getValueAt(index, 2).toString());
				
				representativeDashboardController.sendObjectToModel("INSERT_PRODUCT_SERVICE");
				representativeDashboardController.sendObjectToModel(productServicesHibernateObj);
	...
	private void update() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				if (validateDatatypeInCells() && validateForEmptyCells()) {
					ProductServicesHibernate productServicesHibernateObj = new ProductServicesHibernate();
					productServicesHibernateObj.setId(Integer.parseInt(table.getValueAt(index, 0).toString()));
					productServicesHibernateObj.setProductId(Integer.parseInt(table.getValueAt(index, 1).toString()));
					productServicesHibernateObj.setProductName(table.getValueAt(index, 2).toString());
					
					representativeDashboardController.sendObjectToModel("UPDATE_PRODUCT_SERVICE");
					representativeDashboardController.sendObjectToModel(productServicesHibernateObj);
	...
	private void delete() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				...
				representativeDashboardController.sendObjectToModel("DELETE_PRODUCT_SERVICE");
