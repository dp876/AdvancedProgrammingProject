...
public class ReportedIssueTableInternalView extends JInternalFrame {
	 ...
	 * Create the frame.
	 */
	public ReportedIssueTableInternalView(RepresentativeDashboardController representativeDashboardController) {
		super("Reported Issue", true, true, true, true);
		...
		TimerTask task = new TimerTask() {
	        public void run() {
	        	representativeDashboardController.sendQueryToModel("READ_ALL_REPORTED_ISSUES");
	        }
	    };
	...
	private void initalizeComponents() {
		...
		table.setModel(new DefaultTableModel(
				...
				new String[] {
					"Id", "User Id", "Product Id", "Category Id", "Complaint", "Completed"
				}
			) {
				/**
				 * 
		...
		btnRefresh.setIcon(new ImageIcon(ReportedIssueTableInternalView.class.getResource("/Images/icons8-refresh-document-25.png")));
		btnRefresh.setFocusable(false);
		btnRefresh.setBounds(10, 460, 140, 30);
		
		btnInsert = new JButton("Insert");
		btnInsert.setIcon(new ImageIcon(ReportedIssueTableInternalView.class.getResource("/Images/icons8-insert-button-25.png")));
		btnInsert.setFocusable(false);
		btnInsert.setBounds(160, 460, 140, 30);
		
		btnUpdate = new JButton("Update");
		btnUpdate.setIcon(new ImageIcon(ReportedIssueTableInternalView.class.getResource("/Images/icons8-update-25.png")));
		btnUpdate.setFocusable(false);
		btnUpdate.setBounds(310, 460, 140, 30);
		
		btnDelete = new JButton("Delete");
		btnDelete.setIcon(new ImageIcon(ReportedIssueTableInternalView.class.getResource("/Images/icons8-delete-bin-30.png")));
		btnDelete.setFocusable(false);
		btnDelete.setBounds(460, 460, 140, 30);
		
		btnNewRow = new JButton("New Row");
		btnNewRow.setIcon(new ImageIcon(ReportedIssueTableInternalView.class.getResource("/Images/icons8-plus-25.png")));
	...
	public void populateTable(ArrayList<ReportIssueHibernate> reportedIssueList) {
		DefaultTableModel tableModel = (DefaultTableModel) table.getModel();
		tableModel.setRowCount(0); // clear table

		for (ReportIssueHibernate value : reportedIssueList) {
			tableModel.addRow(new Object[] { value.getId(), value.getUserid(), 
					value.getProductServiceId(), value.getComplaintCategoryId(),
					value.getComplaint(), value.getCompleted() });
		} 
	}
	...
	private void insert() {
		...
		if (table.getValueAt(index, 0) == null) {
			if (validateDatatypeInCells() && validateForEmptyCells()) {
				...
				reportIssueHibernateObj.setUserid(Integer.parseInt(table.getValueAt(index, 1).toString()));
				reportIssueHibernateObj.setProductServiceId(Integer.parseInt(table.getValueAt(index, 2).toString()));
				reportIssueHibernateObj.setComplaintCategoryId(Integer.parseInt(table.getValueAt(index, 3).toString()));
				reportIssueHibernateObj.setComplaint(table.getValueAt(index, 4).toString());
				reportIssueHibernateObj.setCompleted(table.getValueAt(index, 5).toString());
				
				representativeDashboardController.sendObjectToModel("INSERT_REPORTED_ISSUE");
				representativeDashboardController.sendObjectToModel(reportIssueHibernateObj);
	...
	private void update() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				if (validateDatatypeInCells() && validateForEmptyCells()) {
					ReportIssueHibernate reportIssueHibernateObj = new ReportIssueHibernate();
					reportIssueHibernateObj.setId(Integer.parseInt(table.getValueAt(index, 0).toString()));
					reportIssueHibernateObj.setUserid(Integer.parseInt(table.getValueAt(index, 1).toString()));
					reportIssueHibernateObj.setProductServiceId(Integer.parseInt(table.getValueAt(index, 2).toString()));
					reportIssueHibernateObj.setComplaintCategoryId(Integer.parseInt(table.getValueAt(index, 3).toString()));
					reportIssueHibernateObj.setComplaint(table.getValueAt(index, 4).toString());
					...
					representativeDashboardController.sendObjectToModel("UPDATE_REPORTED_ISSUE");
					representativeDashboardController.sendObjectToModel(reportIssueHibernateObj);
	...
	private void delete() {
		...
		if (table.getValueAt(index, 0) != null) {
			if (!table.getValueAt(index, 0).toString().isEmpty()) {
				...
				representativeDashboardController.sendObjectToModel("DELETE_REPORTED_ISSUE");
